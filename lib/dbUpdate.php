...
if($ok!==false && $currDbVersion<$updateToVer){
    ...
    $updateMssg.=dbUpdate_getMssg("","Adding db support for allowed music extension.");
    $updateMssg.=dbUpdate_getMssg("","Adding 'All' option in albums and genre selection.");
    $updateMssg.=dbUpdate_getMssg("","Adding album creation in multi-album per folder.");
    $updateMssg.=dbUpdate_getMssg("","Adding compatibility with SQL4.1 upward.");
    $updateMssg.=dbUpdate_getMssg("","Correcting a few bugs, typing errors.");
...
$updateToVer=20101215;
if($ok!==false && $currDbVersion<$updateToVer){
    $updateNeeded=true;
    $updateMssg.=dbUpdate_getMssg($updateToVer,"Improved update catalogs by folder modified to include catalogs with inserted/deleted files.");
    $updateMssg.=dbUpdate_getMssg("","Add orphans deletion by folder modified in update catalogs.");

    if($ok)$ok=dbUpdate("ALTER TABLE `albums` ADD INDEX `folderPath` (folderPath (255))");
    if($ok!=false && dbUpdate_setDBVer($updateToVer)){
        $currDbVersion=$updateToVer;
    }
}
$updateToVer=20110102;
if($ok!==false && $currDbVersion<$updateToVer){
    $updateNeeded=true;
    $updateMssg.=dbUpdate_getMssg($updateToVer,"Changed the default skin to the side menu (This can be set back in Admin->Preferences Defaults).");
    
    if($ok)$ok=dbUpdate("update configs set value='skins/SideMenu' where configID=10");
    if($ok!=false && dbUpdate_setDBVer($updateToVer)){
        $currDbVersion=$updateToVer;
    }
}

$updateToVer=20111218;

if($ok!==false && $currDbVersion<$updateToVer){
    $updateNeeded=true;
    $updateMssg.=dbUpdate_getMssg($updateToVer,"Added aac/mp4 support.  Note this requires faad(ver 2+) be installed on the server and the proper path set in the system configs.");

    //if($ok)$ok=dbUpdate("update configs set value='skins/SideMenu' where configID=10");
    if($ok)$ok=dbUpdate("Replace configs set configID=46, name='faadCommand', value='/usr/local/bin/faad -q -f 2 -o - %FILE%', type=0, displayType='text', valuesList='', editable=1 ");
    if($ok)$ok=dbUpdate("update configs set value='.mp3, .ogg, .flac, .wma, .wav, .m4a, .mp4, .aac' where configID=45");
    if($ok!=false && dbUpdate_setDBVer($updateToVer)){
        $currDbVersion=$updateToVer;
    }
}
...
$updateToVer=20121111;

if($ok!==false && $currDbVersion<$updateToVer){
    $updateNeeded=true;
    $updateMssg.=dbUpdate_getMssg($updateToVer,"-Fixed some glitches with the tab browsing scroll functions.<br>
-Enabled single line scrolling in the sidetab widget<br>
-Enabled mousewheel scrolling in the sidetab widget (scroll list of artists/albums with mouse wheel)<br>
-fixed the few files that were still using lazy php tags<br>
