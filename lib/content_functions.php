...
function getDefaultContent(){//Main page stuff.
	...
	if($loadPrefs)$html="<script language='JavaScript'>menu_ItemSelected('menu_preferences','editPreferences','',0); </script>";
	else $html=" <script language='JavaScript'>
        ...
        return $html;
    }else return "small screen stuff here.";
}
...
function doSearch($searchVal,$searchType){
    ...
    if(!string_search($searchType,$searchVal))exit;
	...
	bldsql_col("art.artistID");
        ...
        if($a){
            ...
            foreach($ids as $i=>$id){
                ...
                if($lengths[$i])$names[$i].=" (".util_formatSongLength($lengths[$i]).") ";
                if($bitRates[$i])$names[$i].=" - ".util_formatBitRate($bitRates[$i])." ";
            ...
            $html.="</div>";
        }else $html=_conf("lang_noneFound");
        return $title.$html;
    }else{
...
### Alain add select "all" case..break : not needed but...
			...
			//If we're doing a letter search, then attempt to highlight it.
                        if($letterFilter && !$useHaystack)$html.="<script language='JavaScript'>setBrowseFilterSelectedStyle('filter_".$filter."')</script>";//note that the 3rd param follows syntax rules used by the getLetterSelects function below.
...
//songLength
	...
	if($userID!=UL_UID)$html.="<span class='tiny'> ($userName)&nbsp;&nbsp;</span>";
	$html.="</h3>";
...
function getPlayNowLink($type,$id,$altText=""){//Returns the link to play object now.. type can be song, album, artist, playlist or genre
    ...
    }else $html="<a href='".$_SERVER['PHP_SELF']."?doWhat=".$doWhat."&id=$id'>$play</a>";
    if(_conf("isDemoSystem"))$html=$play;    
    return $html;
}
...
function getStoredSessionData($key){
	...
	if($dbSess){
		...
		if(isset($dbSess[$key]))$ret=$dbSess[$key];
	}
	return $ret;
}
...
switch ($searchType){
    ...
    if($url)$url="<a href=\"".$url."\" target='_new'>$image<div class='search'>$label</div></a>";
    return $url;
}
